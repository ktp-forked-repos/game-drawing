...
window.onload = function() {
	...
	console.time("toggle_visibility_1 from onload");
	toggle_visibility('xy2Box');
	console.timeEnd("toggle_visibility_1 from onload");
	console.time("toggle_visibility_2 from onload");
	toggle_visibility('xy2Label');
	console.timeEnd("toggle_visibility_2 from onload");
	console.time("setDefaults from onload");
	Value.setDefaults();
	console.timeEnd("setDefaults from onload");
	Game.cursor = new Image();
	Game.cursor.src = 'cursor.png';
	Game.cursor.onload = function() {
		console.time("update from onload");
		State.update();
		console.timeEnd("update from onload");
		console.log("---------------------------------------------Onload Complete----------");
	};
	console.time("selectResolution from onload");
	Select.resolution('640');
	console.timeEnd("selectResolution from onload");
}
...
State = {
	...
	reset: function() {
		...
		if(Game.points)
			Toggle.tool('points');
		console.time("setDefaults from reset");
		Value.setDefaults();
		console.timeEnd("setDefaults from reset");
		console.time("selectResolution from reset");
		Select.resolution('640');
		console.timeEnd("selectResolution from reset");
		console.time("update from reset");
		this.update();
		console.timeEnd("update from reset");
		return;
	},
	...
	update: function() {
		...
		console.time("updateSliders from update");
		this.updateSliders();
		console.timeEnd("updateSliders from update");
		console.time("draw from update");
		Output.draw();
		console.timeEnd("draw from update");
		return;
	},
	...
	updateFromSliders: function() {
		...
		console.time("update from updateFromSliders");
		this.update();
		console.timeEnd("update from updateFromSliders");
	},
	...
	gameLoop: function() {
		if(Game.on){
			...
			console.time("draw from gameLoop");
			Output.draw();
			console.timeEnd("draw from gameLoop");
...
Value = {
	setDefaults: function() {
		...
		console.time("setFun from setDefaults")
		this.setFun('sin');
		console.timeEnd("setFun from setDefaults")
		console.time("setMode from setDefaults")
		this.setMode('spiral');
		console.timeEnd("setMode from setDefaults")
		return;
	},
	...
	setFun: function(fun) {
		...
		try {
			console.time("update from setFun");
			State.update();
			console.timeEnd("update from setFun");
		} catch (e) {
			if(e instanceof TypeError) {}
		}
		return;
	...
	generateNew: function() {
		console.time("setDefaults from generateNew");
		this.setDefaults();
		console.timeEnd("setDefaults from generateNew");
		Game.globalCount = 0;
	...
	random: function() {
		console.time("generateNew from random");
		this.generateNew();
		console.timeEnd("generateNew from random");
		console.time("update from random");
		State.update();
		console.timeEnd("update from random");
	...
	selectors: function(sel) {
		switch(sel) {
			case 'xy1Box':
				...
				console.time("update from selectors: xy1Box");
				State.update();
				console.timeEnd("update from selectors: xy1Box");
				break;
			...
			case 'xy2Box':
				...
				console.time("update from selectors: xy2Box");
				State.update();
				console.timeEnd("update from selectors: xy2Box");
				break;
			...
			case 'rBox':
				...
				console.time("update from selectors: rBox");
				State.update();
				console.timeEnd("update from selectors: rBox");
				break;
			...
			case 'gBox':
				...
				console.time("update from selectors: gBox");
				State.update();
				console.timeEnd("update from selectors: gBox");
				break;
			...
			case 'bBox':
				...
				console.time("update from selectors: bBox");
				State.update();
				console.timeEnd("update from selectors: bBox");
				break;
			case 'gameCanvas':
				if(Game.mode == 'user') {
			...
			case 'sclBox':
				...
				console.time("update from selectors: sclBox");
				State.update();
				console.timeEnd("update from selectors: sclBox");
				break;
			default:
				console.time("update from selectors: default");
				State.update();
				console.timeEnd("update from selectors: default");
				break;
		}
		return;
	}
...
Toggle = {
		...
		switch(tool) {
			case 'fuzz':
				if(!Game.fuzz) {
					...
					console.time("update from tool: fuzz on");
					State.update();
					console.timeEnd("update from tool: fuzz on");
				}
				else {
					Game.fuzz = false;
					State.docSet_innerHTML('tool_fuzz', "Fuzz");
					document.getElementById('tool_fuzz').style.backgroundColor = '#FFFEB3';
					console.time("update from tool: fuzz off");
					State.update();
					console.timeEnd("update from tool: fuzz off");
				}
				break;
			case 'crush':
				if(!Game.crush) {
					...
					console.time("update from tool: crush on");
					State.update();
					console.timeEnd("update from tool: crush on");
				}
				else {
					Game.crush = false;
					State.docSet_innerHTML('tool_crush', "Crush");
					document.getElementById('tool_crush').style.backgroundColor = '#FFFEB3';
					console.time("update from tool: crush off");
					State.update();
					console.timeEnd("update from tool: crush off");
				}
				break;
			case 'mosaic':
				if(!Game.mosaic) {
					...
					console.time("update from tool: mosaic on");
					State.update();
					console.timeEnd("update from tool: mosaic on");
				}
				else {
					Game.mosaic = false;
					State.docSet_innerHTML('tool_mosaic', "Mosaic");
					document.getElementById('tool_mosaic').style.backgroundColor = '#FFFEB3';
					console.time("update from tool: mosaic off");
					State.update();
					console.timeEnd("update from tool: mosaic off");
				}
				break;
			case 'color':
				if(!Game.color) {
					...
					console.time("update from tool: color on");
					State.update();
					console.timeEnd("update from tool: color on");
				}
				else {
					Game.color = false;
					State.docSet_innerHTML('tool_color', "GrayScale");
					document.getElementById('tool_color').style.backgroundColor = '#FFFEB3';
					console.time("update from tool: color off");
					State.update();
					console.timeEnd("update from tool: color off");
				}
				break;
			case 'points':
				if(!Game.points) {
					...
					console.time("update from tool: points on");
					State.update();
					console.timeEnd("update from tool: points on");
				}
				else {
					Game.points = false;
					...
					toggle_visibility('xy2Box');
					toggle_visibility('xy2Label');
					console.time("update from tool: points off");
					State.update();
					console.timeEnd("update from tool: points off");
				}
				break;
		}
		console.time("update from tool: ");
		State.update();
		console.timeEnd("update from tool: ");
		return;
	},
	...
	animate: function() {
		if(!Game.on) {
			...
			console.time("gameLoop from animate");
			State.gameLoop();
			console.timeEnd("gameLoop from animate");
		}
		else {
			Game.on = false;
			State.docSet_innerHTML('pausePlay', "Play");
			console.time("update from animate");
			State.update();
			console.timeEnd("update from animate");
		}
		return;
	}
}
